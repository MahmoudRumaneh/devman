...
function parameterVariableName(name) {
  const normalized = String(name || '')
    ...
    .replace(/^_+|_+$/g, '')
    .toUpperCase();
  return /^[A-Z_]/.test(normalized) ? normalized : `PARAM_${normalized || 'VALUE'}`;
}

function explicitParameterExample(parameter) {
  if (parameter.example !== undefined) return parameter.example;
  if (!isRecord(parameter.schema)) return undefined;
  ...
  if (parameter.schema.const !== undefined) return parameter.schema.const;
  if (Array.isArray(parameter.schema.enum) && parameter.schema.enum.length) return parameter.schema.enum[0];
  return undefined;
}
...
function buildPathAndHeaders(document, path, parameters) {
  ...
  parameters.map((parameter) => resolveParameter(document, parameter)).filter(Boolean).forEach((parameter) => {
    ...
    if (parameter.in === 'query') {
      ...
      const cookie = `${parameter.name}=${String(value ?? '')}`;
  ...
  parameters.map((parameter) => resolveParameter(document, parameter)).filter(Boolean).forEach((parameter) => {
    ...
    const explicitExample = explicitParameterExample(parameter);
    const value = explicitExample === undefined
      ? `\${${parameterVariableName(parameter.name)}}`
      : encodeURIComponent(String(explicitExample));
    resolvedPath = resolvedPath.split(`{${parameter.name}}`).join(value);
  });
  resolvedPath = resolvedPath.replace(/\{([^{}]+)\}/g, (match, name) =>
    `\${${parameterVariableName(name)}}`);

  if (!query.length) return { path: resolvedPath, headers };
  return { path: `${resolvedPath}${resolvedPath.includes('?') ? '&' : '?'}${query.join('&')}`, headers };
}

function requestBodyObject(document, operation) {
  if (!isRecord(operation.requestBody)) return null;
  if (typeof operation.requestBody.$ref !== 'string') return operation.requestBody;
  const resolved = resolveJsonPointer(document, operation.requestBody.$ref);
  return isRecord(resolved) ? resolved : null;
}
...
function requestBodyExample(document, operation, parameters) {
  const openApiBody = requestBodyObject(document, operation);
  if (isRecord(openApiBody) && isRecord(openApiBody.content)) {
    ...
    if (mediaEntry && isRecord(mediaEntry[1])) {
      return mediaExample(document, mediaEntry[1]);
    }
  }
...
function urlEncodedBodyConfiguration(document, operation) {
  const openApiBody = requestBodyObject(document, operation);
  if (!isRecord(openApiBody) || !isRecord(openApiBody.content)) return null;
  const entry = Object.entries(openApiBody.content)
    .find(([mediaType]) => /^application\/x-www-form-urlencoded(?:;|$)/i.test(mediaType));
  if (!entry || !isRecord(entry[1])) return null;
  const example = mediaExample(document, entry[1]);
  if (!isRecord(example)) return { body: '', headers: { 'Content-Type': entry[0] } };
  const body = Object.entries(example)
    .map(([name, value]) => `${encodeURIComponent(name)}=${encodeURIComponent(String(value ?? ''))}`)
    .join('&');
  return { body, headers: { 'Content-Type': entry[0] } };
}

function textBodyConfiguration(document, operation) {
  const openApiBody = requestBodyObject(document, operation);
  if (!isRecord(openApiBody) || !isRecord(openApiBody.content)) return null;
  const entry = Object.entries(openApiBody.content).find(([mediaType]) =>
    /^(?:text\/[^;]+|application\/(?:xml|.+\+xml|graphql|javascript|x-javascript|yaml|x-yaml|toml|csv))(?:;|$)/i.test(mediaType));
  if (!entry || !isRecord(entry[1])) return null;
  const example = mediaExample(document, entry[1]);
  const body = typeof example === 'string' ? example : JSON.stringify(example ?? '', null, 2);
  return { body, headers: { 'Content-Type': entry[0] } };
}
...
function securityVariableName(name, suffix) {
  const normalized = String(name || '')
    .replace(/([a-z0-9])([A-Z])/g, '$1_$2')
    .replace(/[^A-Za-z0-9_]+/g, '_')
    .replace(/^_+|_+$/g, '')
    .toUpperCase() || 'API';
  if (normalized === suffix || normalized.endsWith(`_${suffix}`)) return normalized;
  return `${normalized}_${suffix}`;
}

function documentSecuritySchemes(document) {
  if (isRecord(document.components) && isRecord(document.components.securitySchemes)) {
    return document.components.securitySchemes;
  }
  return isRecord(document.securityDefinitions) ? document.securityDefinitions : {};
}

function securityConfiguration(document, operation, request) {
  const requirements = operation.security !== undefined ? operation.security : document.security;
  if (!Array.isArray(requirements) || !requirements.length || !isRecord(requirements[0])) {
    return { ...request, secured: false };
  }

  const headers = { ...request.headers };
  let path = request.path;
  let secured = false;
  const schemes = documentSecuritySchemes(document);
  Object.keys(requirements[0]).forEach((schemeName) => {
    const scheme = isRecord(schemes[schemeName]) ? schemes[schemeName] : null;
    if (!scheme) {
      secured = true;
      return;
    }
    ...
    if ((type === 'http' && httpScheme === 'basic') || type === 'basic') {
      headers.Authorization = `Basic \${${securityVariableName(schemeName, 'BASIC_AUTH')}}`;
      return;
    }
    if (type !== 'apikey' || typeof scheme.name !== 'string' || !scheme.name) return;
    const value = `\${${securityVariableName(schemeName, 'API_KEY')}}`;
    if (scheme.in === 'query') {
      path += `${path.includes('?') ? '&' : '?'}${encodeURIComponent(scheme.name)}=${value}`;
    } else if (scheme.in === 'cookie') {
      headers.Cookie = headers.Cookie ? `${headers.Cookie}; ${scheme.name}=${value}` : `${scheme.name}=${value}`;
    } else {
      headers[scheme.name] = value;
    }
  });
  return { path, headers, secured };
}
...
function normalizeOpenApiDocument(document, documentUrl, sourceUrl) {
  ...
  Object.entries(document.paths).forEach(([rawPath, pathItem]) => {
    ...
    Object.entries(pathItem).forEach(([method, rawOperation]) => {
      ...
      const securedRequest = securityConfiguration(document, rawOperation, request);
      ...
      const encodedBody = urlEncodedBodyConfiguration(document, rawOperation);
      const textBody = textBodyConfiguration(document, rawOperation);
      const bodyConfiguration = encodedBody || textBody;
      ...
      operations.push({
        ...
        secured: securedRequest.secured,
        headers: { ...securedRequest.headers, ...(bodyConfiguration?.headers || {}) },
        body: bodyConfiguration?.body ?? (bodyExample === undefined ? '' : JSON.stringify(bodyExample, null, 2)),
