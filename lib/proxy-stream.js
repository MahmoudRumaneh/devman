...
function containsUnsafeHeaderCharacter(value) {
...
function encodeHeaderValue(name, value) {
  if (containsUnsafeHeaderCharacter(value)) {
...
function applyProxyErrorHeaders(response, error) {
  const attempts = Number.isInteger(error?.attempts) && error.attempts > 0
    ? error.attempts
    : 1;
